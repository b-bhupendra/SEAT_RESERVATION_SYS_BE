...
class TestLogin:
    ...
    def test_seeder_populates_rbac_permissions(self, client, db):
        """Ensures running DatabaseSeeder sets up DBRole records so user.permissions is returned."""
        from database.seeders.database_seeder import DatabaseSeeder
        # Override the seeder's internal Session to use the test db session
