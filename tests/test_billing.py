...
class TestBillingList:
    ...
    def test_filter_bills_by_customer_id(self, client, db):
        token = get_admin_token(client, db)
        customer_1 = client.post("/api/customers", json={
            "name": "Customer One",
            "email": "cust1@test.com",
            "phone": "555-0001",
            "status": "active",
        }, headers=auth_headers(token)).json()
        customer_2 = client.post("/api/customers", json={
            "name": "Customer Two",
        ...
        # Create a bill for customer 1 and customer 2
        client.post("/api/bills", json=bill_payload(customer_1["id"]), headers=auth_headers(token))
        client.post("/api/bills", json=bill_payload(customer_2["id"]), headers=auth_headers(token))

        # Query bills filtered by customer_1's ID
        resp = client.get(f"/api/bills?customer_id={customer_1['id']}", headers=auth_headers(token))
        assert resp.status_code == 200
        data = resp.json()
        assert data["total"] == 1
        assert data["items"][0]["customer_id"] == customer_1["id"]

        # Query bills filtered by customer_2's ID
        resp = client.get(f"/api/bills?customer_id={customer_2['id']}", headers=auth_headers(token))
        assert resp.status_code == 200
        data = resp.json()
        assert data["total"] == 1
        assert data["items"][0]["customer_id"] == customer_2["id"]
